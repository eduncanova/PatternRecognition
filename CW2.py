...
#Conducting on training set. To yield accuracy result, execute until line 438.
d=20 #low-dimensional parameter
...
#%%Conducting Mahalanobis on testing set. Same method just different variables.
#Execute until line 545 for full accuracy result.
d=20 #low-dimensional parameter
e=10 #error parameter
...
#%%Mahalanobis implemented with imposter set based on the paper (http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.383.7807&rep=rep1&type=pdf&fbclid=IwAR3M4jEPZcI66wnHfEcxRtbapFHb0GiFmnmtMX6e2IlpcEgXx3dlyN1AAdc), here done with training set
#Run until 615 for full accuracy result
sim_pairs=np.zeros((2048,2048))
diff_pairs=np.zeros((2048,2048))
...
#%%Mahalanobis implemented with imposter set, here done with testing set
#Run until 685 for full accuracy result
sim_pairs=np.zeros((2048,2048))
diff_pairs=np.zeros((2048,2048))

d_ij=np.zeros((1400,5328))
idx_d=[]
for i in range (1400):
    for j in range (5328):
        d_ij[i,j]=(np.sum(((features_query[i,:] - features_gallery[j,:]))**2)) #contain all distances between pairs
        if labels_query[i]!=labels_gallery[j]:
            idx_d.append([i,j]) #stores index within d_ij of dissimilar points
    ...
    for j in range (5):
        if labels_query[i]==labels_gallery[j]:
            ...
            for s in range(len(idx_d)):
                ...
                if d_ij[first,second]<=d_ij[i,j]:#we have identified an imposter invading perimeter of the similar pair
                    w_il=np.exp(-d_ij[first,second]/d_ij[i,j])
                    diff=(w_il*(features_query[i,:]-features_gallery[j,:]).T)*((features_query[i,:]-features_gallery[j,:]))
                    diff_pairs=diff_pairs+diff #covariance of dissimilar points

eigvals, eigvecs = np.linalg.eigh(sim_pairs-diff_pairs) 

#kNN after implementing Mahalanobis on testing set 
get_min = np.zeros(5328)
min_matrix = np.zeros(1400)
accuracy = 0
counter = 0
